...
select max(application_dt)
 from PO_STAGE_DB."Worker".PO_WD_APPLICATION_STG where length(last_recruiting_stage_change_moment) <> 16 limit 10 
;
...
ALTER TABLE PO_STAGE_DB."Worker".PO_WD_APPLICATION_STG ALTER Department SET DATA TYPE VARCHAR(200)
...
pattern = '.*WD_Job_Applications_Work.*_20240404.*csv' 
...
Last_Functionally_Updated_Dt  Varchar(20),
Close_Dt  Varchar(20),
Fill_Dt  Varchar(20),
...
create or replace TABLE PO_HRIS_DB."Application".PO_WD_CANDIDATE (
 ...
 create or replace TABLE PO_HRIS_DB."Application".PO_WD_REQUISITION (
    ...
    //RECRUITER_WORKDAY_ID VARCHAR(128),--check to see add
...
select * from tt_job_req where   req_id =  'JR23-10708';
...
(     SELECT 
       ...
       upper( trim((substr((aps.job_requisition),charindex('JR',(aps.job_requisition),0),charindex(' ',(aps.job_requisition),0))))) Req_ID ,
        ...
        //to_date(aps.REQ_CREATED_MOMENT, 'YYYY/MM/DD HH24:MI')  as Open_Date,
        //aps.REQ_CREATED_MOMENT as Open_Date,
        aps.UNFREEZE_DT,
        aps.FREEZE_DT,
        aps.Close_DT,
       // aps.date_moved_to_readyhire,
        //aps.target_hire_dt_from_offer,
